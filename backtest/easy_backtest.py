...
from easy_signal import signal

def strategy_test(para1,para2,Open,High,Low,Close,Volume):
    ...
    for d in Close.loc["2023-09-06"+' 9:31:00':date[-1]+' 15:00:00',:].index:#每一分钟,第一天和最后一天丢了问题也不大，
        ...
        if d.split(" ")[1]=="14:40:00":#只是预筛选
            ...
            max10_stkcode=signal(para1,para2,d,Close.columns,"14:40:00",cyf,Open,High,Low,Close,Volume)#time=14:40:00 stkcode_list=Close.columns
        ...
        if d.split(" ")[1]=="14:50:00":
            starttime = datetime.datetime.now()
            max10_stkcode1=signal(para1,para2,d,max10_stkcode,"14:50:00",cyf,Open,High,Low,Close,Volume)#time=14:40:00 stkcode_list=Close.columns
            endtime = datetime.datetime.now()
