...
def a_stock_signal(t,d,c,tem_dict,final_min,Open, High, Low, Close,para1,para2,time):            
...
def signal(para1,para2,d,stkcode_list,time,cyf,Open,High,Low,Close):#time=14:40:00 stkcode_list=Close.columns 目前只有做多
    ...
    for t in stkcode_list:#收盘价，每一只股票吧
        #t1 = threading.Thread(target=a_stock_signal, args=(t,str(d),c,tem_dict,final_min,Open, High, Low, Close,para1,para2,time,))
        t1=Process(target=a_stock_signal, args=(t,str(d),c,tem_dict,final_min,Open, High, Low, Close,para1,para2,time,))
        t1.start()
        #a_stock_signal(t,d,c,tem_dict,final_min,Open, High, Low, Close,para1,para2,time)
        t1.join()    
